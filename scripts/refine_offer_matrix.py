...
if css_anchor not in s: raise SystemExit('CSS badge anchor not found')
s=s.replace(css_anchor,css_anchor+css_add,1)
# mobile repeat var too
...
new=r'''function renderOffersFit(r){
  ...
  const offerFloor=x=>{
    const t=norm(x.text);
    const signature=[
      'plusieurs workflows','plusieurs workflow','rythmes differents','plusieurs conditions',
      'reactiver automatiquement','reactivation automatique','fidelisation automatisee','automatiser la fidelisation',
      'segmenter les communications','communication segmentee','type de client','orchestrer plusieurs parcours',
      'orchestration','logique conditionnelle complexe'
    ];
    const plus=[
      'plusieurs parcours','3 parcours','4 parcours','5 parcours','parcours differents','parcours distincts',
      'ressaisie','ressaisies','double saisie','synchronis','plusieurs canaux','coordonner les actions',
      'echeances importantes','relier les echeances','adapter les relances','automatiser plusieurs etapes',
      'etapes reellement differentes','structurer separement'
    ];
  ...
  const catalog={
    essentiel:{key:'essentiel',name:'ESSENTIEL',price:'2 500 € Net',floor:0},
    plus:{key:'plus',name:'ESSENTIEL+',price:'4 000 € Net',floor:1},
    signature:{key:'signature',name:'SIGNATURE',price:'9 000 € Net',floor:2}
  };
  ...
  table.innerHTML=`<div class="offers-fit-row"><div class="offers-fit-cell offers-fit-head">Optimisations identifiées chez vous</div>${visibleOffers.map(o=>`<div class="offers-fit-cell offers-fit-head">${o.name}</div>`).join('')}</div>`+
    items.map(x=>{const p=priority(x);return `<div class="offers-fit-row"><div class="offers-fit-cell"><div class="offers-fit-opt"><strong>${x.text}</strong><small class="offers-fit-badge ${p.cls}">${p.label}</small></div></div>${visibleOffers.map(o=>mark(covered(x,o))).join('')}</div>`}).join('');
  ...
  const desc=o=>{
    if(o.key==='essentiel')return o.key===recommended?'Couvre l’ensemble des besoins identifiés dans un parcours simple.':'Couvre les besoins les plus simples de votre diagnostic.';
    if(o.key==='plus')return o.key===recommended?'Couvre l’ensemble des optimisations identifiées dans votre fonctionnement.':'Va plus loin dans la structuration et l’automatisation.';
    return 'Couvre également les optimisations avancées et les logiques complexes identifiées.';
  };
